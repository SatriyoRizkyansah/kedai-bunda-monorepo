      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            {/* Random Theme */}
            <Button onClick={handleRandomTheme} variant="outline" className="gap-2 w-full sm:w-auto">
              <Shuffle className="h-4 w-4" />
              Random
            </Button>

            {/* Light/Dark Toggle */}
            <Button onClick={() => setVariant(variant === "light" ? "dark" : "light")} variant="outline" className="gap-2 w-full sm:w-auto">
              {variant === "light" ? (
                <>
          ...
          <CardContent className="p-5 sm:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="bg-primary text-primary-foreground p-3 sm:p-4 rounded-xl">
                  <Sparkles className="h-7 w-7 sm:h-8 sm:w-8" />
                </div>
                <div>
                  <p className="text-xs sm:text-sm text-muted-foreground">Tema Aktif</p>
                  <h3 className="text-xl sm:text-2xl font-bold text-foreground">{getThemeDisplayName(theme as ThemeName)}</h3>
                  <p className="text-xs sm:text-sm text-muted-foreground capitalize mt-1">Mode: {variant}</p>
                </div>
              </div>

              <div className="flex items-center gap-2">
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-primary border-2 border-primary-foreground/20" />
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-secondary border-2 border-secondary-foreground/20" />
                <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-accent border-2 border-accent-foreground/20" />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          <Badge variant={selectedCategory === "all" ? "default" : "outline"} className="cursor-pointer shrink-0" onClick={() => setSelectedCategory("all")}>
            Semua ({allThemes.length})
          </Badge>
          {Object.keys(themeCategories).map((category) => (
            <Badge key={category} variant={selectedCategory === category ? "default" : "outline"} className="cursor-pointer capitalize shrink-0" onClick={() => setSelectedCategory(category)}>
        ...
        {/* Theme Grid/List */}
        <div
          className={`${viewMode === "grid" ? "grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4" : "space-y-3"} ${isChanging ? "opacity-50 pointer-events-none" : "opacity-100"} transition-opacity duration-300`}
        >
